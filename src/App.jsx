...
const App = () => {
  ...
  const handleNextLogic = () => {
    ...
    if (questionIndex < questions.length - 1) {
      ...
      evaluateBucket(questions.length);
    }
  };
  ...
  // --- CUSTOM STYLES INJECTED AS COMPONENT ---
  const CustomCSS = () => (
    <style>{`
      .glass-card {
  ...
  if (view === 'start') {
    return (
      <div dir="rtl" className="min-h-screen bg-[#F8FAFC] relative overflow-hidden text-slate-800 font-sans selection:bg-purple-200">
        <CustomCSS />
        
        {/* Abstract Background Shapes */}
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-purple-200/40 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/2 pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-teal-200/40 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/2 pointer-events-none"></div>

        <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-6 text-center">
          {/* Logo Container */}
          <div className="w-28 h-28 bg-gradient-to-tr from-teal-400 to-purple-600 rounded-3xl flex items-center justify-center mb-10 shadow-[0_20px_50px_rgba(126,34,206,0.25)] rotate-6 transform transition-all duration-700 hover:rotate-12 hover:scale-105">
            <Star className="text-white w-14 h-14 drop-shadow-md" fill="white" />
          </div>
          
          <h1 className="text-4xl md:text-5xl font-black text-slate-900 mb-4 tracking-tight">آکادمی زبان بتی</h1>
          <div className="h-1.5 w-16 bg-purple-600 rounded-full mb-6"></div>
          
          <p className="text-slate-500 mb-10 max-w-xs text-lg leading-8 font-medium">
            دقیق‌ترین آزمون تعیین سطح هوشمند. 
            <br/>
            <span className="text-purple-600 font-extrabold bg-purple-50 px-3 py-1 rounded-lg mt-2 inline-block">سطح واقعی</span> خود را بشناسید.
          </p>
          
          {/* Features Grid */}
          <div className="grid grid-cols-1 gap-4 w-full max-w-xs mb-12">
            {[
              {icon: Activity, text: "تطبیق با سطح شما", color: "text-teal-600", bg: "bg-teal-50"},
              {icon: Zap, text: "تحلیل هوشمند و فوری", color: "text-purple-600", bg: "bg-purple-50"},
              {icon: BookOpen, text: "سوالات استاندارد جهانی", color: "text-blue-600", bg: "bg-blue-50"}
            ].map((item, idx) => (
              <div key={idx} className="flex items-center gap-4 glass-card p-4 rounded-2xl shadow-sm transition-transform hover:scale-[1.02]">
          ...
          <button 
            ...
            className="w-full max-w-xs bg-slate-900 text-white font-bold py-5 rounded-2xl shadow-2xl hover:bg-slate-800 transition-all active:scale-95 flex items-center justify-center gap-3 text-lg ring-4 ring-slate-100 group"
          >
            شروع آزمون حرفه‌ای 
  ...
  if (view === 'test') {
    ...
    return (
      <div dir="rtl" className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans selection:bg-purple-200">
        <CustomCSS />
        {/* Header */}
        <div className="bg-white/80 backdrop-blur-md px-6 py-5 shadow-sm flex justify-between items-center sticky top-0 z-20 border-b border-slate-100">
          <div className="flex items-center gap-2">
            <span className="text-xs font-black text-white bg-gradient-to-r from-purple-600 to-indigo-600 px-4 py-1.5 rounded-full shadow-lg shadow-purple-200">
              سطح {currentStage}
            </span>
          </div>
          <span className="text-slate-400 font-mono font-bold text-lg dir-ltr">
            {questionIndex + 1}<span className="text-slate-300 text-sm">/{questions.length}</span>
          </span>
        </div>

        {/* Progress Bar */}
        <div className="w-full bg-slate-100 h-1.5" dir="ltr">
          <div 
            className="bg-purple-600 h-1.5 shadow-[0_0_15px_rgba(147,51,234,0.6)] transition-all duration-500 ease-out rounded-r-full" 
        ...
        <div className="flex-1 flex flex-col p-6 max-w-md mx-auto w-full justify-center">
          {/* Question Card */}
          <div dir="ltr" className="glass-card question-card-shadow p-8 rounded-[2rem] mb-8 relative overflow-hidden group border-0 ring-1 ring-white">
            ...
            <h2 className="text-2xl font-bold text-slate-800 text-center leading-relaxed relative z-10 font-sans">
              {currentQ.text}
            </h2>
          </div>
          ...
          <div className="space-y-4" dir="ltr">
            {currentQ.options.map((opt, idx) => (
              <button
                ...
                className={`w-full p-5 rounded-2xl text-left transition-all duration-300 border-2 font-semibold text-lg flex items-center justify-between group font-sans relative overflow-hidden
                  ${selectedOption === idx 
                    ? 'border-purple-500 bg-purple-50 text-purple-700 shadow-lg shadow-purple-100 scale-[1.02]' 
                    : 'border-transparent bg-white text-slate-600 shadow-sm hover:border-purple-200 hover:bg-purple-50/30'
                  }
                `}
              >
                <span className="relative z-10">{opt}</span>
                {selectedOption === idx ? (
                  <CheckCircle className="w-6 h-6 text-purple-600 drop-shadow-sm relative z-10" />
                ) : (
                  <div className="w-5 h-5 rounded-full border-2 border-slate-200 group-hover:border-purple-300 transition-colors relative z-10"></div>
                )}
              </button>
            ))}
          </div>
  ...
  if (view === 'result') {
    return (
      <div dir="rtl" className="min-h-screen bg-[#F1F5F9] font-sans pb-10 selection:bg-teal-200">
        <CustomCSS />
        ...
        <div className="bg-[#0F172A] text-white pt-12 pb-24 px-6 rounded-b-[3.5rem] shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 text-center animate-fade-in-up">
            ...
            <div className="relative w-64 h-32 mx-auto mb-8 overflow-hidden">
              ...
              <div 
                className={`absolute bottom-0 w-full h-full border-[24px] border-teal-400 rounded-t-full border-b-0 origin-bottom transition-all duration-[2000ms] cubic-bezier(0.34, 1.56, 0.64, 1) shadow-[0_0_40px_rgba(45,212,191,0.5)]`}
                style={{ 
        ...
        <div className="px-5 -mt-16 relative z-20 space-y-5 max-w-lg mx-auto">
          ...
          {/* Card 1: Meaning */}
          <div className="glass-card p-6 rounded-3xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border border-white animate-fade-in-up delay-100">
            <div className="flex items-center gap-3 mb-3 border-b border-slate-100 pb-3">
              <div className="bg-blue-50 p-2.5 rounded-2xl text-blue-600 shadow-inner">
                <BookOpen size={22} />
              </div>
              <h3 className="font-extrabold text-slate-800 text-lg">این سطح به چه معناست؟</h3>
            </div>
            <p className="text-slate-600 leading-7 text-sm text-justify font-medium">
              {finalLevel.desc}
            </p>
          </div>

          {/* Card 2: Analysis */}
          <div className="glass-card p-6 rounded-3xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border border-white animate-fade-in-up delay-200">
            <div className="flex items-center gap-3 mb-3">
              <div className="bg-teal-50 p-2.5 rounded-2xl text-teal-600 shadow-inner">
                <Activity size={22} />
              </div>
              <h3 className="font-extrabold text-slate-800 text-lg">تحلیل تخصصی عملکرد</h3>
            </div>
            <p className="text-slate-600 leading-7 text-sm text-justify font-medium">
              {analysis}
            </p>
          ...
          {/* Card 3: Roadmap */}
          <div className="glass-card p-6 rounded-3xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border border-white animate-fade-in-up delay-300">
            <div className="flex items-center gap-3 mb-3">
              <div className="bg-purple-50 p-2.5 rounded-2xl text-purple-600 shadow-inner">
                <Lock size={22} />
              </div>
              <h3 className="font-extrabold text-slate-800 text-lg">نقشه راه پیشرفت شما</h3>
            </div>
            <p className="text-slate-600 leading-7 text-sm text-justify font-medium">
              {roadmap}
            </p>
          ...
          <div className="mt-8 text-center px-4 animate-fade-in-up delay-300">
             <div className="inline-block bg-teal-50 border border-teal-100 rounded-2xl p-4 shadow-sm">
                 <div className="flex items-center justify-center gap-2 text-teal-700 font-black text-sm mb-2">
                    <TicketPercent className="w-5 h-5" />
                    پیشنهاد ویژه ثبت‌نام
                 </div>
                 <p className="text-slate-700 font-bold text-sm leading-7">
                   با نشان دادن اسکرین‌شات این صفحه به پشتیبانی،
                   <br/>
                   <span className="text-purple-600 font-black text-xl mx-1 bg-purple-100 px-3 py-1 rounded-lg inline-block my-1 border border-purple-200">۲۰٪ تخفیف ویژه</span>
                   <br/>
                    برای ثبت‌نام دوره جامع آکادمی زبان بتی دریافت کنید.
                 </p>
             </div>
          </div>

          {/* Screenshot Button */}
          <div className="pt-6 pb-8 animate-fade-in-up delay-300">
            <div className="relative group cursor-pointer">
              ...
              <div className="relative bg-[#0F172A] rounded-2xl p-1 flex items-center justify-center">
                 <div className="w-full bg-[#0F172A] border border-teal-500/50 rounded-xl py-4 px-6 flex items-center justify-center gap-3 shadow-[inset_0_0_20px_rgba(45,212,191,0.2)]">
                    ...
                    <span className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-teal-200 to-white neon-text tracking-wide">
                      اسکرین‌شات بگیرید
                    </span>
                 </div>
              </div>
          ...
          <div className="text-center pb-2 opacity-50">
            <p className="text-slate-400 text-[10px] tracking-widest uppercase">© Betty Academy 2024</p>
          </div>
